...
const EnhancedUploadForm = ({ onUpload, uploadConfig }) => {
	...
	// Only NITK uploads should capture professor/year
	const isNITK = (uploadConfig?.college || "").toLowerCase() === "nitk";
		...
		try {
			...
			const mergedConfig = {
				...
				tags: tags
					...
					.map((t) => t.trim())
					.filter(Boolean),
				// Only include professor/year for NITK
				...(isNITK ? { professor: professor.trim(), year: year.trim() } : {}),
			...
			const uploadData = {
				...
				tags: tags
					...
					.map((tag) => tag.trim())
					.filter((tag) => tag),
				// Only surface professor/year in callback data for NITK
				...(isNITK
					? {
